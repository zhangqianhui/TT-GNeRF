...
class RegionL1lossOp(torch.nn.Module):
    def __init__(self):
        ...
        self.beard_mask = torch.zeros(size=[1, 3, h, w])
        self.beard_mask[:, :, 40*scale:58*scale, 18*scale:46*scale] = 1
    ...
    def forward(self, input_img, gt1, gt2, input, mask_id=0):
        ...
        if mask_id == 0:
            ...
            id = 17
            input_img = F.interpolate(input_img, (512, 512), mode='bilinear', align_corners=True)
            net = self.net.to(input_img.device)

            i_parsing = net(input_img)[0]
            i_parsing = torch.argmax(i_parsing, dim=1, keepdim=True)
            i_parsing_ = torch.where(i_parsing == id, 1, 0).float()
            i_parsing_ = F.interpolate(i_parsing_, (input.shape[2], input.shape[3]), mode='bilinear', align_corners=True)

            target_loss = self.l1_loss(input * i_parsing_.repeat(1, input.shape[1], 1, 1),
                                       gt2 * i_parsing_.repeat(1, input.shape[1], 1, 1))
            i_parsing_ = 1 - i_parsing_
            nontarget_loss = self.l1_loss(input * i_parsing_.repeat(1, input.shape[1], 1, 1),
                                          gt1 * i_parsing_.repeat(1, input.shape[1], 1, 1))
            ...
            i_parsing = net(input_img)[0]
            i_parsing = torch.argmax(i_parsing, dim=1, keepdim=True)
            i_parsing1_ = torch.where(i_parsing == id1, 1, 0).int()
            i_parsing2_ = torch.where(i_parsing == id2, 1, 0).int()
            i_parsing_ = i_parsing1_ ^ i_parsing2_

            i_parsing_ = F.interpolate(i_parsing_.float(), (input.shape[2], input.shape[3]), mode='bilinear', align_corners=True)
            target_loss = 0.0
            nontarget_loss = self.l1_loss(input * i_parsing_.repeat(1, input.shape[1], 1, 1),
                                          gt1 * i_parsing_.repeat(1, input.shape[1], 1, 1))
            ...
            i_parsing_ = i_parsing1_ ^ i_parsing2_
            i_parsing_ = F.interpolate(i_parsing_.float(), (input.shape[2], input.shape[3]), mode='bilinear', align_corners=True)
            target_loss = self.l1_loss(input * i_parsing_, gt1 * i_parsing_)
            ...
            input_img = F.interpolate(input_img, (512, 512), mode='bilinear', align_corners=True)
            net = self.net.to(input_img.device)
            parsing = net(input_img)[0]
            parsing = torch.argmax(parsing, dim=1, keepdim=True)
            background = torch.where(parsing == id, 1, 0).float()
            background = F.interpolate(background, (input.shape[2], input.shape[3]), mode='bilinear',
                                       align_corners=True)

            i_parsing_ = (1 - i_parsing_) - background
            nontarget_loss = self.l1_loss(input * i_parsing_, gt1 * i_parsing_)
...
class RegionL1loss_Normal(torch.nn.Module):
    def __init__(self):
        ...
        self.smile_mask[:, 36*scale:48*scale, 14*scale:50*scale, :] = 1
        self.beard_mask = torch.zeros(size=[1, h, w, 3])
        self.beard_mask[:, 40*scale:58*scale, 18*scale:46*scale, :] = 1
