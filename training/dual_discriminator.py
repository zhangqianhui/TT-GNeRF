...
#         if c_dim > 0:
#             self.mapping = MappingNetwork(z_dim=0, c_dim=c_dim, w_dim=cmap_dim, num_ws=None, w_avg_beta=None, **mapping_kwargs)
#         self.b4 = DiscriminatorEpilogue(channels_dict[4], cmap_dim=cmap_dim, label_dim=label_dim, resolution=4, **epilogue_kwargs, **common_kwargs)
...
class DualDiscriminator(torch.nn.Module):
    ...
    def forward(self, img, c, update_emas=False, **block_kwargs):
        ...
        normal_editing = depth2normal(img['image_depth'].squeeze(1)).permute(0, 3, 1, 2)
        image_normal = filtered_resizing(normal_editing, size=img['image'].shape[-1], f=self.resample_filter)
        img = torch.cat([img['image'], image_normal], 1)
