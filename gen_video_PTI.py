...
def gen_interp_video(G, mp4: str, seeds, shuffle_seed=None, w_frames=60 * 4, kind='cubic', grid_dims=(1, 1),
                     num_keyframes=None, wraps=2, psi=1, truncation_cutoff=14, cfg='FFHQ', image_mode='image', output_dir=None,
    ...
    styles = torch.load(os.path.join(os.path.join(output_dir, '{}.pt'.format(1000)))).repeat([1, 14, 1])
    styles = styles.repeat(num_keyframes, 1, 1)
    ws = styles.reshape(grid_h, grid_w, num_keyframes, *styles.shape[1:])
    ...
    for yi in range(grid_h):
        ...
        for xi in range(grid_w):
            ...
            y = np.tile(ws[yi][xi].cpu().detach().numpy(), [wraps * 2 + 1, 1, 1])
...
def generate_images(
    ...
    if interpolate:
        ...
        gen_interp_video(G=G, mp4=output, bitrate='10M', grid_dims=grid, num_keyframes=num_keyframes, w_frames=w_frames,
                         ...
                         truncation_cutoff=truncation_cutoff, cfg=cfg, image_mode=image_mode, gen_shapes=shapes, output_dir=outdir)
    else:
        for seed in seeds:
            ...
            gen_interp_video(G=G, mp4=output, bitrate='10M', grid_dims=grid, num_keyframes=num_keyframes,
                             ...
                             truncation_cutoff=truncation_cutoff, cfg=cfg, image_mode=image_mode, output_dir=outdir)
