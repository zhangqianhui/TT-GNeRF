...
def accumulate(model1, model2, decay=0.999, isforediting=False):
...
def make_mm(batch, mm_dim, device):
    mm = torch.randn(batch, mm_dim, device=device)
    return mm
...
def make_one_hot_label(batch, latent_dim, device):
    labels = torch.randint(0, latent_dim, (batch,), device=device)
    labels = torch.nn.functional.one_hot(labels, num_classes=latent_dim)
    id = torch.randint(0,2,(1,), device=device)
    if 0 in id:
        return labels
    else:
        return - labels

def make_specific_one_hot_label(batch, latent_dim, device):

    att = torch.randint(0,2,(1,), device=device)

    if att == 0:
        label = torch.randint(0, 1, (batch,), device=device) + 4
    else:
        label = torch.randint(0, 1, (batch,), device=device) + 5

    label = torch.nn.functional.one_hot(label, num_classes=latent_dim)
    id = torch.randint(0,2,(1,), device=device)
    if 0 in id:
        return label
    else:
